...
class RliableConfig:
    ...
    score_normalization_method: Literal["maxmin", "atari", "onezero", "100zero", "procgen"] = "maxmin"
    ...
    normalized_score_threshold_min: Optional[float] = None
    """the minimum threshold for the normalized score for the performance profile"""
    sample_efficiency_and_walltime_efficiency_method: Optional[
        Literal["Median", "IQM", "Mean", "Optimality Gap"]] = "IQM"
    """the method to compute the sample efficiency and walltime efficiency"""
    combined_figure: bool = True
    """if toggled, we will generate the cover figure with IQM"""
    ...
    plot_probability_of_improvement: bool = False
    """if toggled, we will plot the probability of improvement"""
    sample_efficiency_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the sample efficiency"""
    performance_profile_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the performance profile"""
    interval_estimates_num_bootstrap_reps: int = 10000  # 50000
    """the number of bootstrap replications in `rliable` to use for computing the the interval estimates"""
    aggregate_fig_title: str = None
    """the title of the aggregate figure"""
...
class Runset:
    def __init__(
        ...
        include_seeds_groups = [{"config.seed.value": {"$in": self.seeds}}] if len(self.seeds) > 0 else []
        self.wandb_filters = {
            "$and": [
                {f"config.{self.custom_env_id_key}.value": self.env_id},
                *include_tag_groups,
                *user,
                *include_seeds_groups,
...
def create_hypothesis(runset: Runset, scan_history: bool = False) -> Hypothesis:
    ...
    for idx, run in enumerate(runset.runs):
        ...
        if scan_history:
            ...
            run_df = run.history(samples=10000000, keys=["global_step", "_runtime", runset.metric])

        if "videos" in run_df:
            run_df = run_df.drop(columns=["videos"], axis=1)
        if len(runset.metric) > 0:
            try:
                # normalize the metric given the ranges for the environment
                env_id = runset.env_id
                if "distribution_mode" in runset.runs[0].config:
                    if runset.runs[0].config["distribution_mode"] == "easy":
                        procgen_hns = progcen_easy_hns
                    else:
                        procgen_hns = progcen_hard_hns
                ...
                import pdb
                pdb.set_trace()
        # run_df["charts/episodic_return"] = run_df["charts/episodic_return"].max()
...
def compare(
    ...
    for idx, env_id in enumerate(env_ids):
        ...
        # for runsets in runsetss:
        #     if "Training" in runsets[idx].name:
        #         runsets[idx].linestyle = "dotted"

        # for runsets in runsetss:
        #     if "easy" in runsets[idx].name:
        #         runsets[idx].linestyle = "dotted"

        # for runsets in runsetss:
        #     if "Impala" in runsets[idx].name:
        #         runsets[idx].linestyle = "dotted"

        for runsets in runsetss:
            if "GAP" in runsets[idx].name:
                runsets[idx].linestyle = "solid"
            else:
                runsets[idx].linestyle = "dotted"

        #
        # marker = [runsets[idx].marker for runsets in runsetss]
        linestyle = [runsets[idx].linestyle for runsets in runsetss]
        # exp_names = [runsets[idx].exp_name for runsets in runsetss]

        # # update matplotlib.rcParams['axes.prop_cycle'] to hack the linestyle
        ...
        ex.plot(
            ...
            # make dotted lines for the y-axis
        )

        # set matplotlib.rcParams['axes.prop_cycle'] back to solid
        plt.rcParams['axes.prop_cycle'] = plt.cycler(linestyle=['-'])

        ax.set_xlabel("")
        ax.set_ylabel("")
        # captalize the first letter of the title
        ax.set_title(r'\textbf{' + env_id[0].upper() + env_id[1:] + '}',
                     fontsize='small')  # Insert variable into LaTeX bold formatting
        ...
        # ax.yaxis.set_major_locator(MultipleLocator(0.1))  # Y-axis tick steps
    ...
    print_rich_table(f"Runtime ({pc.time_unit}) (mean ± std)", runtimes.rename_axis("Environment").reset_index(),
                     console)
    ...
    # change the style of every second plot in the figure to dotted lines
    # they should also have the same color as the solid lines

    # for idx,line in enumerate(axes_flatten[0].get_lines()):
    #     if idx % 2 == 1:
    #         line.set_linestyle("--")
    #         # set the color of the dotted line to the color of the solid line
    #         line.set_color(axes_flatten[0].get_lines()[idx-1].get_color())
    ...
    fig.legend(
        ...
        # loc="upper left",
        ncol=10,
        # ncol=pc.ncols_legend * (pc.ncols // 2),
        # ncol=1,
    ...
    fig.supxlabel(pc.xlabel, fontsize='large', y=0.02)
    fig.supylabel(pc.ylabel, fontsize='large', x=0.02)
    fig.tight_layout()

    title_name = args.output_filename.split('/')[-1]
    title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'

    if args.rc.aggregate_fig_title is not None:
        title_name += f' {args.rc.aggregate_fig_title}'
    # set the overall title of the figure
    # fig.suptitle(title_name, fontsize='large', y=1.1)

    h, l = axes_time_flatten[0].get_legend_handles_labels()
    fig_time.legend(
        h, l, loc="lower center", ncol=pc.ncols_legend, bbox_to_anchor=(0.5, 0.95), bbox_transform=fig_time.transFigure,
        frameon=False
    )
...
def maxmin_normalize_score(score_dict: Dict[str, np.ndarray]):
    ...
    max_scores = all_scores.max(0).max(1)  # 1) max over all experiments and seeds 2) max over all steps
    min_scores = all_scores.min(0).min(1)  # 1) min over all experiments and seeds 2) min over all steps
    return normalize_score(score_dict, max_scores, min_scores)


def onezero_normalize_score(score_dict: Dict[str, np.ndarray], upper_value=1):
    all_scores = np.concatenate([score_dict[key] for key in score_dict], axis=0)
    max_scores = all_scores.max(0).max(1)  # 1) max over all experiments and seeds 2) max over all steps
    min_scores = all_scores.min(0).min(1)  # 1) min over all experiments and seeds 2) min over all steps
    one_scores = np.ones_like(max_scores) * upper_value
    zero_scores = np.zeros_like(min_scores)
    return normalize_score(score_dict, one_scores, zero_scores)
...
def procgen_normalize_score(score_dict: Dict[str, np.ndarray], env_ids):
    max_scores = np.array([procgen_hns[env_id][2] for env_id in env_ids])
    min_scores = np.array([procgen_hns[env_id][1] for env_id in env_ids])
    return normalize_score(score_dict, max_scores, min_scores)
...
if __name__ == "__main__":
    ...
    c_1 = colors_flatten_original[1]
    # colors_flatten_original[1] = colors_flatten_original[5]
    # colors_flatten_original[5] = c_1
    del colors_flatten_original[5]

    # For generalization plot, hack the color and marker cycle
    colors = []
    # for a, b in zip(colors_flatten_original[:-1], colors_flatten_original[1:]):
    #     colors.append(a)
    #     colors.append(b)
    # colors_flatten_original = colors

    # for a in colors_flatten_original:
    #     colors.append(a)
    #     colors.append(a)
    # colors_flatten_original = colors

    markers_flatten_original = \
        ["o", "s", "^", "*", "D", "v", "<", ">", "p", "P", "*", "X", "d", "H", "h", "+", "x", "|", "_"]
    plt.rcParams["axes.prop_cycle"] = plt.cycler("color", colors_flatten_original)
    plt.rcParams["axes.prop_cycle"] = plt.cycler("marker", markers_flatten_original)
    colors_flatten = copy.deepcopy(colors_flatten_original)
    markers_flatten = copy.deepcopy(markers_flatten_original)
    colors = []
    markers = []
    ...
    for filters in args.filters:
        ...
        markers += [markers_flatten[: len(filters) - 1]]
        markers_flatten = markers_flatten[len(filters) - 1:]
    ...
    for filters_idx, filters in enumerate(args.filters):
        ...
        if f"{wandb_entity}/{wandb_project_name}" not in offline_dbs:
            offline_db_folder = os.path.join(openrlbenchmark.__path__[0], "dataset",
                                             f"{wandb_entity}/{wandb_project_name}")
        ...
        for filter_str, color, marker in zip(filters[1:], colors[filters_idx], markers[filters_idx]):
            print("=========", filter_str)
            # parse filter string
            ...
            seeds = [int(q) for q in query["seed"]] if "seed" in query else []
            ...
            for env_id in args.env_ids[filters_idx]:
                ...
                if args.check_empty_runs:
                    console.print(
                        f"{exp_name} [green]({query})[/] in [purple]{env_id}[/] has {len(runsets[-1].runs)} runs")
    ...
    if args.rliable:
        ...
        for runsets_idx, runsets in enumerate(runsetss):
            score_dict[runsets[0].name] = np.zeros(
                (min_num_seeds_per_hypothesis[runsets[0].name], len(args.env_ids[0]), args.rc.nsubsamples + 1)
            )
            step_dict[runsets[0].name] = np.zeros(
                (min_num_seeds_per_hypothesis[runsets[0].name], len(args.env_ids[0]), args.rc.nsubsamples + 1)
            )
            # for each seed
            for seed_idx, _ in enumerate(range(min_num_seeds_per_hypothesis[runsets[0].name])):  # exs[0][runsets_idx]
                min_global_step = float("inf")
                print(f"collecting runs for {runsets[0].name} seed idx {seed_idx}")
                ...
                for ex_idx, ex in enumerate(exs):
                    ...
                    x_samples = np.linspace(
                        min(run_of_one_seed.df["global_step"]), max(run_of_one_seed.df["global_step"]),
                        num=args.rc.nsubsamples
                    )

                    score_dict[runsets[0].name][seed_idx, ex_idx, 1:] = np.interp(
                        x_samples, run_of_one_seed.df["global_step"], run_of_one_seed.df["charts/episodic_return"]
                    )
                    step_dict[runsets[0].name][seed_idx, ex_idx, 1:] = x_samples
        ...
        markers_flatten = markers_flatten_original
        markers = dict(zip(list(score_dict.keys()), markers_flatten))
        ...
        elif args.rc.score_normalization_method == "onezero":
            normalized_score_dict = onezero_normalize_score(score_dict)
        elif args.rc.score_normalization_method == "100zero":
            normalized_score_dict = onezero_normalize_score(score_dict, 100)
        elif args.rc.score_normalization_method == "procgen":
            normalized_score_dict = procgen_normalize_score(score_dict, args.env_ids[0])
        else:
        ...
        if args.rc.combined_figure:
            print("plotting combined efficiency curve")
            ...
            metric_fn = metrics.aggregate_iqm
            metric_name = "IQM"
            # metric_fn = metrics.aggregate_median
            # metric_name = "Median"
            ...
            for exp_name in score_dict.keys():
                ...
                global_step_xaxis = step_dict[exp_name][0, 0]

                # make - line when training is contained in exp_name, else make -- line
                if "Testing" in exp_name:
                    linestyles = {exp_name: "-"}
                    # make sure that testing uses the same color as the training line
                    colors[exp_name] = colors[exp_name.replace("Testing", "Training")]
                    markers[exp_name] = markers[exp_name.replace("Testing", "Training")]
                elif "Training" in exp_name:
                    # dotted
                    linestyles = {exp_name: "dotted"}
                else:
                    linestyles = None

                # For dormant neuron plot
                # if "Impala" in exp_name:
                #     linestyles = {exp_name: "dotted"}
                # else:
                #     linestyles = {exp_name: "solid"}
                # global_step_xaxis = global_step_xaxis[2:]
                # aggregate_scores[exp_name] = aggregate_scores[exp_name][2:]
                # aggregate_cis[exp_name] = aggregate_cis[exp_name][:, 2:]
                ...
                plot_utils.plot_sample_efficiency_curve(
                    ...
                    ax=ax,
                    # ylabel=metric_name,
                    ylabel=metric_name if args.pc.ylabel is None else args.pc.ylabel + f" ({metric_name})",
                    labelsize="large",
                    ticklabelsize="small",
                    marker=markers,
                    # line style, iterate over the line styles
                    linestyles=linestyles,

                    # markevery=[5, 10, 15, 20, 25],
                    # linewidth=1,
                    # markersize=4,
                )
            # ax.set_yticks(np.linspace(0, args.rc.normalized_score_threshold))
            ax.set_ylim(0, top=args.rc.normalized_score_threshold)

            # get the max globalstep
            max_global_step = max(max_global_steps.values())
            # round to the nearest million
            max_global_step = np.round(max_global_step, -6)

            # ax.set_xticks(np.linspace(5e6, max_global_step, 5))
            ax.set_xticks(np.linspace(0, max_global_step, 11))

            # get title name as the last part of the output_filename that is separated by '/'
            title_name = args.output_filename.split('/')[-1]
            title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'
            # bold title
            if args.rc.aggregate_fig_title is not None:
            ...
            # ax.set_title(f"{title_name}", fontsize="large")
            ax.set_xlabel("")
            ...
            # fig_sample_efficiency.supxlabel(args.pc.xlabel, fontsize='large')  #, fontsize="large")
            # set x lavel to args.pc.xlabel
            ax.set_xlabel(args.pc.xlabel, fontsize="large")
            fig_sample_efficiency.tight_layout()
            # fig_sample_efficiency.savefig(f"{args.output_filename}_combined.png", bbox_inches="tight")
            fig_sample_efficiency.savefig(f"{args.output_filename}_combined.pdf", bbox_inches="tight")
            # tikzplotlib.save(f"{args.output_filename}_combined.tex")
    ...
    if args.rc.sample_efficiency_plots:
        ...
        for metric_fn, ax, metric_name in zip(metric_fns, axes_sample_efficiency.flatten(), metric_names):
            ...
            if metric_name == args.rc.sample_efficiency_and_walltime_efficiency_method:
                ...
                for exp_name in score_dict.keys():
                    ...
                    runtime_xaxis = np.linspace(0, runtime, args.rc.nsubsamples + 1)
                    plot_utils.plot_sample_efficiency_curve(
                        runtime_xaxis,
                        {exp_name: aggregate_scores[exp_name]},
                        {exp_name: aggregate_cis[exp_name]},
        ...
        fig_sample_efficiency.supxlabel(args.pc.xlabel, fontsize="large")
        fig_sample_efficiency.tight_layout()
    ...
    if args.rc.plot_probability_of_improvement:
        # Load ProcGen scores as a dictionary containing pairs of normalized score

        # create pairs of the entries in the performance_profile_normalized_score_dict
        compare_score_dict = {}
        for key1 in performance_profile_normalized_score_dict:
            for key2 in performance_profile_normalized_score_dict:
                if key1 != key2:
                    # check if the inverse is not already in the dictionary
                    if key1 + ',' + key2 not in compare_score_dict:
                        combined_key = key2 + ',' + key1
                        if 'Impala,Impala w/ MoE' not in combined_key:
                            compare_score_dict[combined_key] = (
                                performance_profile_normalized_score_dict[key2],
                                performance_profile_normalized_score_dict[key1])

        # flip the keys in the last dictionary
        # key = 'Impala w/ MoE,Impala'
        # key_old = 'Impala,Impala w/ MoE'
        # compare_score_dict[key] = (compare_score_dict[key_old][1], compare_score_dict[key_old][0])
        # # delte the old key
        # del compare_score_dict[key_old]

        # colors
        colors_probablity = colors_flatten_original[:len(compare_score_dict)]

        # matrices for pairs of algorithms we want to compare
        average_probabilities, average_prob_cis = rly.get_interval_estimates(
            compare_score_dict, metrics.probability_of_improvement, reps=2000)
        axes = plot_utils.plot_probability_of_improvement(
            average_probabilities, average_prob_cis, xlabel=r'P(X $>$ Y)',
            figsize=(2.5, 0.37 * len(compare_score_dict)),
            colors=colors_probablity,
            left_ylabel=r'\textbf{X}',
            right_ylabel=r'\textbf{Y}',
            ticklabelsize='large',
            labelsize='large',
            interval_height=0.6,
            ylabel_x_coordinate=0.3
        )
        plt.savefig(f"{args.output_filename}_probability_of_improvement.pdf", bbox_inches="tight")
    ...
    if args.rc.aggregate_metrics_plots:
        ...
        aggregate_scores_df = pd.DataFrame.from_dict(
            aggregate_scores, orient="index", columns=["IQM"]  # , "Median", "IQM", "Mean", "Optimality Gap"]
        )
        ...
        aggregate_scores_df_all = pd.DataFrame.from_dict(
            aggregate_scores_all, orient="index", columns=["Median", "IQM", "Mean", "Optimality Gap"]
        )
        print_rich_table(f"Aggregate Scores", aggregate_scores_df_all.reset_index(), console)

        # format this in a single table column by combining the median and upper lower confidence intervals
        save_results = aggregate_scores_df.reset_index()
        save_results["IQM"] = save_results["IQM"].map("{:.2f}".format)
        ...
        fig, axes = plot_utils.plot_interval_estimates(
            ...
            aggregate_scores,
            aggregate_score_cis,
            metric_names=["IQM"],  # "IQM", "Mean", "Optimality Gap"],
        ...
        # set x min max
        axes.set_xlim(args.rc.normalized_score_threshold_min, args.rc.normalized_score_threshold)

        # align the yticks labels to the left side
        # for label in axes.get_yticklabels():
        #     label.set_ha('left')

        # axes[1].set_xlabel("Normalized Score", fontsize="large")
        axes.set_xlabel(f"Final {args.pc.ylabel} (IQM)", fontsize="large")
        # add title "Summary of Aggregate Metrics"
        title_name = args.output_filename.split('/')[-1]
        title_name = r'\textbf{' + title_name[0].upper() + title_name[1:] + '}'
        ...
        # import pdb
        # pdb.set_trace()
        # axes.set_title(f"{title_name}", fontsize="large")
