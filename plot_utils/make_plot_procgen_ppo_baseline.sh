python -m rlops \
    ...
    --env-ids bigfish starpilot dodgeball ninja jumper caveflyer bossfight climber leaper jumper \
    --no-check-empty-runs \
    --pc.ncols 3 \
    --pc.ncols-legend 3 \
    --rliable \
    ...
    --output-filename paper_plots/procgen/ppo/baseline/training/training \
    --rc.nsubsamples 10 \
...
for file in ./paper_plots/procgen/ppo/baseline/training/*.pdf; do pdfcrop $file $file;  done
...
python -m rlops \
    ...
    --rc.combined_figure \
    --pc.ylabel "Normalized Score" \
    --output-filename paper_plots/procgen/ppo/baseline/testing/testing \
    --rc.nsubsamples 10 \
    --rc.aggregate_fig_title "Procgen" \
    --metric_last_n_average_window 1 \
    --pc.rm 2.5
...
for file in ./paper_plots/procgen/ppo/baseline/testing/*.pdf; do pdfcrop $file $file;  done
