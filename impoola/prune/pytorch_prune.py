...
class BasePruningMethod(ABC):
    ...
    @abstractmethod
    def compute_mask(self, t, default_mask, topk=None):
    ...
    @classmethod
    def apply(cls, module, name, *args, importance_scores=None, topk=None, **kwargs):
        ...
        try:
            ...
            mask, topk = method.compute_mask(importance_scores, default_mask=default_mask, topk=topk)
    ...
    def prune(self, t, default_mask=None, importance_scores=None, topk=None):
        ...
        return t * self.compute_mask(importance_scores, default_mask=default_mask, topk=topk)[0]
...
class PruningContainer(BasePruningMethod):
    ...
    def add_pruning_method(self, method):
        ...
        # WARNING: Speed up by not appending to a tuple but overwriting it
        self._pruning_methods = (method,)  # type: ignore[operator]
    ...
    def compute_mask(self, t, default_mask, topk=None):
        ...
        * for 'global', the mask will be computed across all entries.
        ...
        def _combine_masks(method, t, mask, topk=None):
            ...
            partial_mask, topk = method.compute_mask(t[slc], default_mask=mask[slc], topk=topk)
            new_mask[slc] = partial_mask.to(dtype=new_mask.dtype)

            return new_mask, topk

        method = self._pruning_methods[-1]
        mask, topk = _combine_masks(method, t, default_mask, topk)
        return mask, topk
...
class RandomUnstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask, topk=None):
        ...
        if nparams_toprune != 0:  # k=0 not supported by torch.kthvalue
            ...
            if topk is None:
                topk = torch.topk(prob.view(-1), k=nparams_toprune).indices
            mask.view(-1)[topk] = 0

        return mask, topk

    @classmethod
    def apply(cls, module, name, amount, topk=None):
        r"""Add pruning on the fly and reparametrization of a tensor.
        ...
        Args:
            ...
            amount (int or float): quantity of parameters to prune.
                ...
                absolute number of parameters to prune.
        """
        return super().apply(module, name, amount=amount, topk=topk)
...
class L1Unstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask, topk=None):
        ...
        if nparams_toprune != 0:  # k=0 not supported by torch.kthvalue
            ...
            if topk is None:
                topk = torch.topk(torch.abs(t).view(-1), k=nparams_toprune, largest=False).indices
    ...
    @classmethod
    def apply(cls, module, name, amount, importance_scores=None, topk=None):
        r"""Add pruning on the fly and reparametrization of a tensor.
        ...
        """
        return super().apply(
            module, name, amount=amount, importance_scores=importance_scores, topk=topk
        )
...
class RandomStructured(BasePruningMethod):
    ...
    @classmethod
    def apply(cls, module, name, amount, dim=-1, topk=None):
        r"""Add pruning on the fly and reparametrization of a tensor.
        ...
        Args:
            ...
            dim (int, optional): index of the dim along which we define
                channels to prune. Default: -1.
        """
        return super().apply(module, name, amount=amount, dim=dim, topk=topk)
...
class LnStructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask, topk):
        r"""Compute and returns a mask for the input tensor ``t``.
        ...
        # largest=True --> top k; largest=False --> bottom k
        # Keep the smallest k channels along dim=self.dim  !!! BREAKING CHANGE TO PYTORCH !!!
        if topk is None:
            topk = torch.topk(norm, k=nparams_toprune, largest=False).indices
        # topk will have .indices and .values
        ...
        def make_mask(t, dim, indices):
            ...
            slc[dim] = indices
            # use slc to slice mask and replace all its entries with 0
            # e.g.: mask[:, :, [0, 2, 3]] = 0
            mask[slc] = 0
            return mask
        ...
        if nparams_toprune == 0:  # k=0 not supported by torch.kthvalue
            ...
            mask = make_mask(t, self.dim, topk)
            mask *= default_mask.to(dtype=mask.dtype)
        return mask, topk

    @classmethod
    def apply(cls, module, name, amount, n, dim, importance_scores=None, topk=None):
        r"""Add pruning on the fly and reparametrization of a tensor.
...
def l1_unstructured(module, name, amount, importance_scores=None, topk=None):
    ...
    """
    _, topk = L1Unstructured.apply(
        module, name, amount=amount, importance_scores=importance_scores, topk=topk
    )
    return module, topk


def random_structured(module, name, amount, dim, topk=None):
    ...
    Examples:
        ...
        3
    """
    # TODO: add support for topk
    RandomStructured.apply(module, name, amount, dim, topk)
    return module, topk


def ln_structured(module, name, amount, n, dim, importance_scores=None, topk=None):
    ...
    Examples:
        ...
        ... )
    """
    _, topk = LnStructured.apply(
        module, name, amount, n, dim, importance_scores=importance_scores, topk=topk
    )
    return module, topk
